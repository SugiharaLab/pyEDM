...
def MakeBlock( dataFrame,
    ...
    DF = gmn.AuxFunc.PandasDataFrametoDF( dataFrame )
    
    # D is a Python dict from pybind11 < cppEDM Embed
    D = pyBindEDM.gmn.MakeBlock( DF,
                             E, 
                             tau,
                             columnNames,
                             verbose )
...
def Embed( pathIn    = "./",
    ...
    if gmn.AuxFunc.NotStringIterable( columns ) :
...
def Simplex( pathIn       = "./",
    ...
    if gmn.AuxFunc.NotStringIterable( lib ) :
        lib = ' '.join( map( str, lib ) )
    if gmn.AuxFunc.NotStringIterable( pred ) :
        pred = ' '.join( map( str, pred ) )
    if gmn.AuxFunc.NotStringIterable( columns ) :
        columns = ' '.join( map( str,columns   ) )
    
    # D is a Python dict from pybind11 < cppEDM Simplex 
    #  { "predictions" : {}, "knn_neighbors" : {}, "knn_library" : {}  }
    ...
    # JP df = DataFrame( D ) # Convert to pandas DataFrame
    df_pred  = DataFrame( D['predictions']   ) # Convert to pandas DataFrame
    df_neigh = DataFrame( D['knn_neighbors'] ) # Convert to pandas DataFrame
    df_lib   = DataFrame( D['knn_library']   ) # Convert to pandas DataFrame

    if showPlot :
        # JP pyEDM.AuxFunc.PlotObsPred( df, dataFile, E, Tp )
        gmn.AuxFunc.PlotObsPred( df_pred, dataFile, E, Tp )
    
    # JP return df
    
    SDict = { 'predictions'   : df_pred,
              'knn_neighbors' : df_neigh,
              'knn_library'   : df_lib }
