...
//helper method to parse a date or time string into a tm obj
...
//helper method to parse the datetime std::string into a struct tm
...
//method to get a new datetime + delta past the range of given
//time column std::strings
...
std::string increment_datetime_str ( std::string datetime1, 
    ...
    //if only diff is in ms, still want some delta update
    if ( seconds_diff == 0 ) { 
        seconds_diff = 1; 
    }
