...
//-----------------------------------------------------------------------
// Convert DFI struct to cppEDM DataFrame< size_t >
// 
...
DataFrame< size_t > DFIToDataFrame( DFI df ) {
    ...
    DataFrame< size_t > dataFrame( numRows, colNames.size(), colNames );
...
//---------------------------------------------------------------
// Convert cppEDM DataFrame< size_t > to DF struct.
// 
// Note : cppEDM DataFrame stores time vector in dataFrame.Time()
//        and label in dataFrame.TimeName()
//---------------------------------------------------------------
DFI DataFrameToDFI( DataFrame< size_t > dataFrame ) {
    ...
    for ( std::string colName : dataFrame.ColumnNames() ) {
        ...
        std::valarray< size_t > colData = dataFrame.VectorColumnName( colName );
    ...
    if ( not dataFrame.ColumnNames().size() ) {
        for ( size_t colidx = 0; colidx < dataFrame.NColumns(); colidx++ ) {
            ...
            std::valarray< size_t > colData = dataFrame.Column( colidx );
...
//---------------------------------------------------------------
// Convert DFI struct to Python dict
//---------------------------------------------------------------
py::dict DFItoDict( DFI df ) {
